...
app.layout = html.Div([
    ...
    html.Br(),
    html.Div([
        html.H5("Sampled data"),
        html.Div(dte.DataTable(rows=[{}], id='table-s'))
    ]),
...
#        df['pf Act'] = pf_True
        ...
        return dt.to_json(date_format='iso', orient='split')
    else:
        return [{}]


@app.callback(Output('table-s', 'rows'), [Input('submit-button', 'n_clicks')],
              [State('practical_sample_value', 'children')])
def update_table(n_clicks, jsonified_cleaned_data):
    if jsonified_cleaned_data is not None:
        dff = pd.read_json(jsonified_cleaned_data, orient='split')
        return dff.to_dict('records')
...
@app.callback(Output('graph-4', 'figure'),
              [Input('submit-button', 'n_clicks')],
              [State('down_sample_value', 'children'), State('practical_sample_value', 'children')])
def update_graph_4(n_clicks, data_1, data_2):
    # Figure for sampled data (10 units each)
    if data_1 is not None and data_2 is not None:
        df_1 = pd.read_json(data_1, orient='split')
        df_2 = pd.read_json(data_2, orient='split')
